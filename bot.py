...
# ══════════════════════════════════════════════
#  Persistance — PostgreSQL (fallback fichier local)
# ══════════════════════════════════════════════
def _db_conn():
        ...
        if os.path.exists(DATA_FILE):
            try:
                with open(DATA_FILE, "r") as f:
                    data = json.load(f)
                logger.info("Données chargées (fichier local).")
            except Exception as e:
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == ChatType.PRIVATE:
        ...
        wl = state.get("whitelist", [])
        wl_sigs = state.get("whitelist_signatures", [])
        ...
        sigs_str = ""
        if wl_sigs:
            sigs_str = "\nSignatures exemptées :\n" + "\n".join(f"  • {s}" for s in wl_sigs)
        ...
        await update.message.reply_text(
            ...
            f"Whitelist IDs : {len(wl)} personne(s)\n"
            f"Whitelist signatures : {len(wl_sigs)} personne(s){sigs_str}"
        )
        return
...
async def cmd_learnsig(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Active le mode apprentissage pour capturer la signature automatiquement."""
    if update.effective_chat.type != ChatType.PRIVATE:
        await update.message.reply_text("Utilise cette commande en privé.")
        return
    user_id = update.effective_user.id
...
async def cmd_addsigfwd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Ajoute une signature via un message transféré depuis le canal."""
    if update.effective_chat.type != ChatType.PRIVATE:
        await update.message.reply_text("Utilise cette commande en privé.")
        return
    user_id = update.effective_user.id
    ...
    if not context.args:
        await update.message.reply_text(
            "Usage : /addsigfwd <channel_id>\n\n"
            "Ensuite transfère un message de la personne à exempter."
        )
        return
    try:
        channel_id = int(context.args[0])
    ...
    if key not in data["channels"] or data["channels"][key].get("owner_id") != user_id:
        await update.message.reply_text("❌ Canal non trouvé.")
        return
    learning_mode[f"fwd:{key}"] = user_id
    await update.message.reply_text(
        "📩 *Mode forward activé !*\n\n"
        "Transfère maintenant un message de la personne\n"
        "que tu veux exempter depuis ton canal.\n\n"
        "Le bot lira sa signature automatiquement.",
        parse_mode="Markdown",
    )


async def handle_forward_sig(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Reçoit un message transféré en mode addsigfwd."""
    msg = update.message
    if not msg or not update.effective_user:
        return
    user_id = update.effective_user.id
    ...
    if fwd_key not in learning_mode or learning_mode[fwd_key] != user_id:
        return

    sig = msg.forward_signature
    if not sig:
        await msg.reply_text(
            "❌ Ce message n'a pas de signature.\n\n"
            "La personne doit avoir activé 'Signer les messages'\n"
            "dans ses paramètres d'admin du canal."
        )
        del learning_mode[fwd_key]
        return

    wl = data["channels"][key].setdefault("whitelist_signatures", [])
    if sig not in wl:
        wl.append(sig)
        save_data()
        await msg.reply_text(
            f"✅ *Signature ajoutée !*\n\n"
            f"Nom détecté : *{sig}*\n\n"
            f"Cette personne ne sera plus supprimée.",
            parse_mode="Markdown",
        )
    else:
        await msg.reply_text(
            f"⚠️ *{sig}* est déjà dans la whitelist.",
            parse_mode="Markdown",
        )
    del learning_mode[fwd_key]
...
async def post_init(app):
    ...
    commands = [
        ...
        BotCommand("learnsig",        "🎓 Apprendre ma signature (en postant)"),
        BotCommand("addsigfwd",       "📩 Ajouter signature via message transféré"),
...
def main():
    ...
    app.add_handler(CommandHandler("addsigfwd",       cmd_addsigfwd))
